...
const InventorySummary = () => {
  ...
  useEffect(() => {
    fetchInventorySummary(options[selectedIndex]);
    fetchProfileInfo();
  }, [selectedIndex]);
  ...
  const fetchInventorySummary = async (itemGroup) => {
    try {
      const response = await axios.get('http://localhost:8080/inventory-item/inventory-summary', {
        params: { itemGroup },
      });
        ...
        <div>
          ...
          <section className="flex flex-row gap-10 ml-6">
            ...
            <ul className='flex flex-col gap-2'>
              ...
              <li>{formattedDateTime.split(' ')[0]}</li>
            ...
            <TableBody>
              {inventoryItems.map((item, index) => (
                <TableRow key={item.itemId}>
                  ...
                  <TableCell align="right">{item.itemDetails}</TableCell>
